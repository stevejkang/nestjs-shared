...
import { ExternalId } from '../src/ExternalId';
...
describe('ExternalId', () => {
  ...
  describe('decode rejects tampered external IDs', () => {
    ...
    it('should return null when re-encode does not match the input', () => {
      const encoded = ExternalId.encode(42, 'user');
      const padded = '0'.repeat(4) + encoded;

      const decoded = ExternalId.decode(padded, 'user');
